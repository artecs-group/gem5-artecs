 ...
 * non-contiguous) DMA transfers on a SgaDmaPort completes.  Derived classes
 * must implement the process() method and use getChunkEvent() to allocate a
 * callback event for each participating DMA.
 */
class SgaDmaCallback : public Drainable
{
  public:
    virtual const std::string name() const { return "SgaDmaCallback"; }

    /**
     * SgaDmaPort ensures that all oustanding DMA accesses have completed
     * before it finishes draining.  However, SgaDmaChunkEvents scheduled with
     * a delay might still be sitting on the event queue.  Therefore, draining
     * is not complete until count is 0, which ensures that all outstanding
     * SgaDmaChunkEvents associated with this SgaDmaCallback have fired.
     */
    DrainState
    drain() override
    {
  ...
  protected:
    ...
    virtual ~SgaDmaCallback() = default;
  ...
  public:
     ...
     * request that wishes to participate in this SgaDmaCallback.
     */
    Event *
    getChunkEvent()
    {
